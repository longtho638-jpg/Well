 ...
 *
 * This is the single entry point for Edge Functions / API routes
 * that receive payment webhooks.
 */
export async function orchestrateBillingWebhook(
  ...
  if (webhookResult.status === 'processed') {
    // Try to parse event again for metadata (lightweight — cached by provider)
