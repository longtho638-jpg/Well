...
export function exportNetworkTreeCSV(rootNode: NetworkNode, filename?: string): void {
  ...
  const columns: CSVColumn[] = [
    { key: 'id', header: 'User ID', formatter: (id: unknown) => typeof id === 'string' ? id.slice(0, 8) : '' },
    ...
    { key: 'rank', header: 'Rank' },
    {
      ...
      formatter: (amount: unknown) => typeof amount === 'number' ? new Intl.NumberFormat('vi-VN').format(amount || 0) : '0'
    },
    {
    ...
    },
    {
      key: 'joinedAt',
      header: 'Joined Date',
      formatter: (date: unknown) => typeof date === 'string' && date !== 'N/A' ? new Date(date).toLocaleDateString('vi-VN') : 'N/A'
    },
    { key: 'parentId', header: 'Parent ID', formatter: (id: unknown) => typeof id === 'string' && id ? id.slice(0, 8) : 'Root' },
