...
import {
  LayoutDashboard, FileText, Users, Wallet, ChevronLeft, ChevronRight,
  Settings, Sparkles, DollarSign, Package, ClipboardList, ShieldCheck, Activity
} from 'lucide-react';
...
interface AdminSidebarNavDesktopProps {
...
export function AdminSidebarNavDesktop({
  sidebarCollapsed,
  setSidebarCollapsed,
  isActive,
  onNavigate
}: AdminSidebarNavDesktopProps) {
  ...
  return (
    ...
    >
      ...
      <div className="h-24 flex items-center justify-between px-8 border-b border-white/5 relative overflow-hidden">
        ...
        {!sidebarCollapsed && (
          <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-4 relative z-10">
            <div className="w-10 h-10 bg-[#00575A] rounded-xl flex items-center justify-center shadow-[0_0_20px_rgba(0,87,90,0.3)]">
              <ShieldCheck className="text-emerald-400 w-6 h-6" />
            </div>
            <div>
              <h1 className="font-black text-white uppercase italic tracking-tighter text-lg leading-tight">{t('admin.mission')}</h1>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all relative group ${
                active ? 'bg-zinc-900 border border-white/10 text-white shadow-2xl' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5'
              }`}
            >
              {active && (
                <motion.div layoutId="active-pill" className="absolute left-0 w-1.5 h-6 bg-teal-500 rounded-r-full shadow-[0_0_15px_rgba(20,184,166,0.6)]" />
              )}
              <span className={`transition-transform duration-500 group-hover:scale-110 ${active ? 'text-teal-400' : ''}`}>{item.icon}</span>
              {!sidebarCollapsed && (
                <>
                  <span className="font-black text-[11px] uppercase tracking-[0.2em] flex-1 text-left italic">{item.label}</span>
                  {item.badge && (
                    <span className={`px-2.5 py-1 text-[9px] font-black rounded-lg border italic ${
                      active ? 'bg-teal-500/10 border-teal-500/20 text-teal-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-500'
                    }`}>
                      {item.badge}
                    </span>
                  )}
      ...
      <div className="p-8 border-t border-white/5 bg-zinc-950/50 backdrop-blur-sm space-y-4">
        ...
        >
          ...
          {!sidebarCollapsed && <span className="font-black text-[11px] uppercase tracking-[0.2em] italic">{t('admin.b_ng_i_u_khi_n')}</span>}
        </button>
        <button
        ...
        {!sidebarCollapsed && (
          <div className="flex items-center gap-4 mt-4 px-2 group cursor-pointer">
            <div className="w-12 h-12 bg-zinc-900 rounded-2xl flex items-center justify-center text-teal-400 font-black border border-white/5 shadow-2xl group-hover:border-teal-500/30 transition-all">A</div>
