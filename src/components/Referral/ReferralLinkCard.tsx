...
export const ReferralLinkCard: React.FC<ReferralLinkCardProps> = ({
    ...
    >
        ...
        <div className="grid grid-cols-2 gap-4">
            <ShareButton icon={MessageCircle} label="Zalo" color="bg-blue-600" onClick={onShareZalo} />
            <ShareButton icon={Facebook} label="Facebook" color="bg-blue-700" onClick={onShareFB} />
            <ShareButton icon={MessageCircle} label="Telegram" color="bg-sky-500" onClick={onShareTelegram} />
            <ShareButton icon={Mail} label="Email" color="bg-rose-500" onClick={onShareEmail} />
        </div>
        ...
        >
            <QrCode size={18} />
            {showQRCode ? 'Dismiss Visual Key' : 'Generate Visual Key'}
        </motion.button>
    </motion.div>
);
