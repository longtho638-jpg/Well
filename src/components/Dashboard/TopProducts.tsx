...
export const TopProducts: React.FC<Props> = ({ products }) => {
  ...
  return (
    ...
    >
      <h3 className="font-bold text-lg text-brand-dark dark:text-slate-100 mb-1">{t('topproducts.top_products')}</h3>
      <p className="text-gray-400 dark:text-slate-500 text-xs mb-6">{t('topproducts.based_on_units_sold')}</p>
            ...
            <div key={product.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between mb-1">
                        ...
                        <p className="text-xs font-medium text-gray-500 dark:text-slate-400">{product.salesCount} {t('topproducts.sold')}</p>
                    </div>
                    ...
                    <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-1 text-right">{t('topproducts.earn')}{formatVND(product.price * product.commissionRate)}</p>
                </div>
            </div>
        ))}
      </div>
