    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="md:col-span-1 lg:col-span-2 bg-gradient-to-br from-brand-primary to-teal-900 dark:from-slate-800 dark:to-slate-900 rounded-2xl p-8 text-white shadow-xl relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start mb-8 relative z-10">
            <div>
              <p className="text-teal-200 dark:text-slate-300 text-sm uppercase tracking-wider font-medium mb-1">Withdrawable Balance</p>
          ...
          <div className="grid grid-cols-2 gap-8 border-t border-white/10 dark:border-slate-600/50 pt-6 relative z-10">
            <div>
              <p className="text-teal-300 dark:text-slate-300 text-xs mb-1">Total Earnings (Gross)</p>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex-1 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col justify-center items-center text-center">
            ...
            <h4 className="font-bold text-gray-800 dark:text-slate-100 mb-1">Tax Compliance Mode</h4>
            <p className="text-xs text-gray-500 dark:text-slate-400 leading-relaxed">
              WellNexus automatically deducts <span className="font-bold text-gray-700 dark:text-slate-300">10% PIT</span> for income exceeding 2,000,000 VNĐ per Vietnam Law.
            </p>
          </div>
          <button
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
        <div className="p-6 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
          <h3 className="font-bold text-gray-800 dark:text-slate-100">Earnings History</h3>
          <button
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
              {processedTransactions.map((t) => (
                <tr key={t.id} className="hover:bg-gray-50/80 dark:hover:bg-slate-700/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-medium text-gray-900 dark:text-slate-100">{t.date}</div>
                    <div className="text-xs text-gray-400 dark:text-slate-500">{t.id}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${t.type.includes('Bonus') ? 'bg-purple-500 dark:bg-purple-400' : 'bg-blue-500 dark:bg-blue-400'}`}></div>
                        <span className="text-gray-700 dark:text-slate-300 font-medium">{t.type}</span>

                        {/* WOW: Larger Badge for Bee Rewards */}
                        {t.metadata?.trigger_agent === 'the_bee' && (
                          <motion.div
                          ...
                          >
                            ...
                            <span className="text-[10px] font-bold text-yellow-700 dark:text-yellow-300 uppercase tracking-wide">The Bee</span>
                          </motion.div>
                        )}
                      </div>

                      {/* WOW TRACEABILITY BADGE */}
                      {t.metadata?.source_tx_id && (
                        <div className="flex items-center gap-1 ml-4">
                          <span className="text-[10px] text-gray-400 dark:text-slate-500 uppercase tracking-wider">Source:</span>
                      ...
                      {t.metadata?.calculation_base && t.metadata?.applied_rate && (
                  ...
                  <td className="px-6 py-4 text-right text-gray-600 dark:text-slate-400 font-medium">{formatVND(t.amount)}</td>
                  <td className="px-6 py-4 text-right text-red-600 dark:text-red-400">{(t.taxDeducted && t.taxDeducted > 0) ? `-${formatVND(t.taxDeducted)}` : '-'}</td>
                  <td className="px-6 py-4 text-right font-bold text-brand-primary dark:text-teal-400">{formatVND(t.amount - (t.taxDeducted || 0))}</td>
                  <td className="px-6 py-4 text-center">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${t.status === 'completed' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400'}`}>{t.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
