export const admin = {
  admin: {
    title: "System Administration",
    subtitle: "Configure and manage the entire system",
    sidebarTitle: "Mission Control",
    nav: {
      overview: "Overview",
      content: "Content",
      partners: "Partners",
      finance: "Finance",
      orders: "Orders",
      products: "Products",
      strategy: "Strategy",
      auditLog: "Audit Log",
      licenses: "Licenses",
      analytics: "Analytics",
      metering: "Metering",
    },
    adminLabel: "Admin",
    superUser: "Super User",
    mission: "Mission",
    control_center: "Control Center",
    ai_sentinel_active: "AI Sentinel Active",
    monitoring_2_4k_identity_nodes: "Monitoring 2.4K identity nodes",
    b_ng_i_u_khi_n: "Dashboard",
    administrator: "Administrator",
    superuser_node: "Superuser Node",
    mission_control: "Mission Control",
    administration: "Administration",
    secure_session: "Secure Session",
    tabs: {
      overview: "Overview",
      cms: "CMS",
      partners: "Partners",
      finance: "Finance",
      strategy: "Strategy",
    },
    overview: {
      title: "System Overview",
      subtitle: "Real-time platform metrics and health status",
      totalRevenue: "Total Revenue",
      activePartners: "Active Partners",
      pendingPayouts: "Pending Payouts",
      systemHealth: "System Health",
      chartTitle: "7-Day Revenue Trend",
    },
    cms: {
      title: "Content Management",
      subtitle: "Edit landing page content and marketing copy",
      heroHeadline: "Hero Headline",
      heroSubheadline: "Hero Subheadline",
      ctaButtonText: "CTA Button Text",
      saveChanges: "Save Changes",
      saveSuccess: "CMS Content saved successfully!",
      preview: "Preview",
      stat_content_banners: "Content Banners",
      stat_live_banners: "Live Banners",
      stat_broadcasts: "Broadcasts",
      stat_total_templates: "Total Templates",
    },
    partners: {
      title: "Partner Operations",
      subtitle: "Manage partner accounts and KYC verification",
      tableHeaders: {
        id: "ID",
        name: "Name",
        rank: "Rank",
        sales: "Sales",
        status: "Status",
        actions: "Actions",
      },
    },
    finance: {
      title: "Financial Operations",
      subtitle: "Manage withdrawal requests and tax compliance",
      export_json: "Export JSON",
      export_csv: "Export CSV",
      gross: "Gross:",
      tax: "Tax (10%):",
      net: "Net:",
      approve: "Approve",
      reject: "Reject",
      noRequests: "No withdrawal requests at this time",
    },
    users: {
      title: "User Management",
      totalUsers: "Total Users",
      activeUsers: "Active",
      verifiedUsers: "Verified",
      bannedUsers: "Banned",
      actions: {
        view: "View",
        edit: "Edit",
        ban: "Ban",
        unban: "Unban",
        verify: "Verify",
        delete: "Delete",
      },
    },
    products: {
      title: "Product Management",
      addProduct: "Add Product",
      editProduct: "Edit Product",
      deleteProduct: "Delete Product",
      activeProducts: "Active",
      draftProducts: "Draft",
    },
    policy: {
      title: "Policy System",
      subtitle: "Manage rules and policies",
      commission: "Commission Policy",
      tax: "Tax Policy",
      rank: "Rank Policy",
      referral: "Referral Policy",
      rules: "Rules",
      active: "Active",
      inactive: "Inactive",
      createRule: "Create Rule",
      editRule: "Edit Rule",
    },
    settings: {
      title: "System Settings",
      general: "General",
      security: "Security",
      notifications: "Notifications",
      integrations: "Integrations",
      backup: "Backup",
    },
    analyze_all: "Analyze All",
    automated_fraud_detection: "Automated Fraud Detection",
    export_ledger: "Export Ledger",
    ledger_synchronized: "Ledger Synchronized",
    no_items_in_the_current_filter: "No items in the current filter",
    platform_liquidity_verificatio: "Platform Liquidity Verification",
    quarantined_items: "Quarantined Items",
    security_batch_commit: "Security Batch Commit",
    security_passed: "Security Passed",
    treasury_control: "Treasury Control",
    verifying_digital_ledgers: "Verifying Digital Ledgers",
    action: "Action",
    content_orchestrator: "Content Orchestrator",
    create: "Create",
    cross_platform_content_deliver: "Cross-Platform Content Delivery",
    link: "Link",
    loc: "LOC",
    target: "Target",
    partner_recon_crm: "Partner Recon CRM",
    precision_orchestration_of_net: "Precision Orchestration of Network",
    rank_intelligence: "Rank Intelligence",
  },
  cms: {
    content_orchestrator: "Content Orchestrator",
    cross_platform_content_deliver: "Cross-Platform Content Delivery",
    create: "Create",
    loc: "LOC",
    action: "Action",
    link: "Link",
    target: "Target",
    tab_banners: "Banners",
    tab_announcements: "Announcements",
    tab_templates: "Templates",
    search_placeholder: "Search by keywords...",
    archive: "Archive",
    enable: "Enable",
    stat_content_banners: "Content Banners",
    stat_live_banners: "Live Banners",
    stat_broadcasts: "Broadcasts",
    stat_total_templates: "Total Templates",
  },
  auditlog: {
    audit_trail: "Audit Trail",
    immutable_ledger_of_administra: "Immutable ledger of administrative actions",
    export_dataset: "Export Dataset",
    analyze_all_actions: "Analyze All Actions",
    export_json: "Export JSON",
    export_csv: "Export CSV",
    all_administrators: "All Administrators",
    timeline: "Timeline",
    operator: "Operator",
    classification: "Classification",
    resource_node: "Resource Node",
    tracing: "Tracing",
    no_telemetry_signals_detected: "No telemetry signals detected",
    event_inspection: "Event Inspection",
    detailed_forensics_for_trace: "Detailed forensics for trace",
    temporal_signature: "Temporal Signature",
    utc_synchronization_active: "UTC Synchronization Active",
    network_origin: "Network Origin",
    browser_api_gateway_node: "Browser / API Gateway Node",
    payload_metadata: "Payload Metadata",
    close_inspection: "Close Inspection",
  },
  finance: {
    treasury_control: "Treasury Control",
    platform_liquidity_verificatio: "Platform Liquidity Verification",
    automated_fraud_detection: "Automated Fraud Detection",
    export_ledger: "Export Ledger",
    analyze_all: "Analyze All",
    security_passed: "Security Passed",
    quarantined_items: "Quarantined Items",
    security_batch_commit: "Security Batch Commit",
    verifying_digital_ledgers: "Verifying digital ledgers...",
    ledger_synchronized: "Ledger Synchronized",
    no_items_in_the_current_filter: "No items in the current filter",
  },
  ordermanagement: {
    cashflow_hub: "Cashflow Hub",
    verify_transactions_and: "Verify transactions and",
    activate_commissions: "activate commissions",
    syncing_global_ledgers: "Syncing global ledgers...",
    queue_synchronized: "Queue Synchronized",
    all_pending_orders_have_been_p: "All pending orders have been processed",
    operational_risk_protocol: "Operational Risk Protocol",
    strict_compliance_rule: "Strict Compliance Rule",
    never_approve: "Never approve",
    without_verified_bank_clearanc: "without verified bank clearance",
  },
  overview: {
    risk: "Risk",
    ai: "AI",
    confident: "Confident",
    resolve: "Resolve",
    reject: "Reject",
    mission_control: "Mission Control",
    autonomous_ecosystem_orchestra: "Autonomous Ecosystem Orchestration",
    ecosystem_online: "Ecosystem Online",
    ai_action_center: "AI Action Center",
    autonomous_recommendations: "Autonomous Recommendations",
    queue_exhausted: "Queue Exhausted",
    ai_agent_has_autonomously_reso: "AI agent has autonomously resolved all items",
    live_pulse: "Live Pulse",
    growth_trajectory: "Growth Trajectory",
    ecosystem_scale: "Ecosystem Scale",
  },
  partners: {
    partner_recon_crm: "Partner Recon CRM",
    precision_orchestration_of_net: "Precision Orchestration of Network",
    rank_intelligence: "Rank Intelligence",
  },
  policyEngine: {
    synchronizingPolicyCore: "Synchronizing Policy Core...",
    title: "Policy Engine",
    version: "Version",
    strategicIntegrityConfirmed: "Strategic Integrity Confirmed",
    sync: "Sync",
    projectionSimulator: "Projection Simulator",
    realTime: "Real-Time",
    policyChangesAreCryptographicallySigned: "Policy changes are cryptographically signed",
  },
  bulkactionsbar: {
    activate: "Activate",
    ban: "Ban",
    export_csv: "Export CSV",
    partner: "Partner",
    selected: "Selected",
  },
  debuggerpage: {
    empty: "Empty",
    environment_window: "Environment Window",
    local_storage_keys: "Local Storage Keys",
    system_debugger: "System Debugger",
    v_debug_1_0: "v.Debug 1.0",
    window_props: "Window Props",
    zustand_store_state: "Zustand Store State",
  },
  testpage: {
    active: "Active",
    client_status: "Client Status",
    connectivity_check: "Connectivity Check",
    well_test_page: "Well Test Page",
  },
  simulationpanel: {
    active_distribution_node: "Active Distribution Nodes",
    margin: "Margin",
    monthly_fixed: "Monthly Fixed",
    nodes: "Nodes",
    projected_ebitda: "Projected EBITDA",
    projected_gmv: "Projected GMV",
    system_payout: "System Payout",
    target_aov: "Target AOV",
    vc_simulation_engine: "VC Simulation Engine",
    strategic_forecast: "Strategic Forecast",
    strategic_candidates: "Strategic Candidates",
    projected_saas_revenue: "Projected SaaS Revenue",
    partners: "Partners",
  },
  fraudbadge: {
    critical_risk: "Critical Risk",
    suspected: "Suspected",
    verified: "Verified",
  },
  partnerfilters: {
    search_placeholder: "Search identity by name, email, or UID...",
    filter_all: "All Identity Nodes",
    filter_active: "Active Status",
    filter_banned: "Banned Status",
    filter_dormant: "Dormant Status",
  },
  adminsecuritysettings: {
    b_o_m_t_t_i_kho_n: "Account Security",
    qu_n_l_c_i_t_b_o_m_t_c_a_b: "Manage your security settings",
    i_m_b_o_m_t: "Security Score",
    b_t_2fa_t_ng_i_m: "Enable 2FA to increase score",
    x_c_th_c_2_y_u_t: "Two-Factor Authentication",
    b_t_2fa_b_o_v: "Enable 2FA for protection",
    th_m_l_p_b_o_m_t: "Extra Security Layer",
    m_t_kh_u: "Password",
    i_l_n_cu_i: "Last Changed",
    i_m_t_kh_u: "Change Password",
    c_nh_b_o_ng_nh_p: "Login Alerts",
    nh_n_th_ng_b_o_khi_c_ng_nh: "Get notified on new sign-ins",
    th_i_gian_phi_n: "Session Duration",
    t_ng_ng_xu_t_sau: "Auto sign-out after",
    ph_t: "minutes",
    l_ch_s_ng_nh_p: "Login History",
    ho_t_ng: "Activity",
    thi_t_l_p_2fa: "Set Up 2FA",
    qu_t_m_qr_v_i_ng_d_ng_x_c_th: "Scan QR code with your authenticator app",
    h_y: "Cancel",
    x_c_nh_n: "Confirm",
  },
  mission_control: "Mission Control",
  nav: {
    overview: "Nav overview",
    content: "Nav content",
    partners: "Nav partners",
    finance: "Nav finance",
    orders: "Nav orders",
    products: "Nav products",
    strategy: "Nav strategy",
    auditLog: "Nav auditLog",
  },
  mission: "Mission",
  control_center: "Control Center",
  ai_sentinel_active: "Ai Sentinel Active",
  monitoring_2_4k_identity_nodes: "Monitoring 2 4k Identity Nodes",
  b_ng_i_u_khi_n: "B Ng I U Khi N",
  administrator: "Administrator",
  superuser_node: "Superuser Node",
  administration: "Administration",
  secure_session: "Secure Session",
  licenses: {
    tier: "Tier",
    basic: "Basic",
    premium: "Premium",
    enterprise: "Enterprise",
    master: "Master",
    suspend: "Suspend",
    unsuspend: "Unsuspend",
    revoke: "Revoke",
    change_tier: "Change Tier",
    view_audit_log: "View Audit Log",
    quota_usage: "Quota Usage",
    api_calls: "API Calls",
    tokens: "Tokens",
    create_title: "Create New License",
    metering: {
      title: "Metering & Overages",
      subtitle: "Track system-wide usage and overage revenue",
      export_csv: "Export CSV",
      date_range: "Date Range",
      this_month: "This Month",
      last_month: "Last Month",
      custom: "Custom",
      usage_table: "Organization Usage Table",
      org_name: "Organization Name",
      plan_tier: "Plan Tier",
      api_usage: "API %",
      booking_usage: "Booking %",
      reports_usage: "Reports %",
      email_usage: "Email %",
      revenue: "Overage Revenue",
      revenue_this_month: "Overage revenue this month",
      top_consumers: "Top Consumers",
      top_5_by_metric: "Top 5 organizations by metric",
      no_data: "No usage data available",
      export_success: "Usage report exported",
    },
    tier_label: "License Tier",
    quota_preset: "Quota Preset",
    expires_label: "Expiration Date",
    user_label: "User ID (or Email)",
    user_placeholder: "Enter user email",
    user_not_found: "User not found with email",
    user_required: "User ID or email is required",
    expires_required: "Expiration date is required",
    list_title: "License List",
    all: "All",
    no_licenses: "No licenses",
    status: "Status",
    actions: "Actions",
    user: "User",
    license_key: "License Key",
    quota_limit_preview: "Quota Preset",
    api_calls_label: "API Calls",
    tokens_label: "Tokens",
    creating: "Creating...",
    created: "Created",
    create_button: "Create License",
    close: "Close",
    success_title: "License created successfully!",
    copy_key_instruction: "Copy this key and send to user",
    suspend_confirm: "Confirm license suspension",
    revoke_confirm: "Confirm license revocation",
    suspend_reason_placeholder: "Enter suspend reason...",
    revoke_reason_placeholder: "Enter revoke reason...",
    cancel: "Cancel",
    enter_suspend_reason: "Enter suspend reason",
    enter_revoke_reason: "Enter revoke reason",
    page_description: "Manage license keys, activate, revoke and view audit logs",
    status_active: "Active",
    status_expired: "Expired",
    status_revoked: "Revoked",
    status_pending: "Pending",
    status_suspended: "Suspended",
    error: "Error",
  },
};
