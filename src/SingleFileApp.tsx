...
interface MarketplaceViewProps {
   onBuy: (product: MockProduct) => void;
}
         ...
         <nav className="flex-1 px-4 py-4 space-y-1">
            {[
               ...
               <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium transition ${view === item.id ? 'bg-[#00575A] text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'
                     }`}
               >
...
// > 3.4 WALLET VIEW
const WalletView = ({ transactions }: WalletViewProps) => {
...
const MarketplaceView = ({ onBuy }: MarketplaceViewProps) => {
...
const SingleFileApp = () => {
   ...
   const handleBuy = async (product: MockProduct) => {
